...
import {  listPaymentsMainAccountOfferAppSharer } from '../../../../../actions/Fund/paymentActions';


const PaymentListMainAccountOfferForAppSharerScreen = (props) => {

    const userSignIn = useSelector((state) => state.userSignIn)
    const { userInfo } = userSignIn;

    const paymentListMainAccountOfferAppSharer = useSelector((state) => state.paymentListMainAccountOfferAppSharer)
    const { payments } = paymentListMainAccountOfferAppSharer;

    const dispatch = useDispatch();

    useEffect(() => {
        dispatch(listPaymentsMainAccountOfferAppSharer(userInfo.item1.phoneNumber))
        return () => {
            //
        };
    }, []);
...
export default PaymentListMainAccountOfferForAppSharerScreen;
