...
import {  listPaymentsTableCashOfferAppSharer } from '../../../../../actions/Fund/paymentActions';



const PaymentListTableCashOfferForAppSharerScreen = (props) => {

    const userSignIn = useSelector((state) => state.userSignIn)
    const { userInfo } = userSignIn;

    const paymentListTableCashOfferAppSharer = useSelector((state) => state.paymentListTableCashOfferAppSharer)
    const { payments } = paymentListTableCashOfferAppSharer;

    const dispatch = useDispatch();

    useEffect(() => {
        dispatch(listPaymentsTableCashOfferAppSharer(userInfo.item1.phoneNumber))
        return () => {
            //
        };
    }, []);
...
export default PaymentListTableCashOfferForAppSharerScreen;
